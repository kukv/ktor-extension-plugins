...
class Environment private constructor(private val delegate: ApplicationConfig) {
    @PublishedApi
    internal fun <T> getTypedPropertyOrDefault(
        key: String,
        defaultValue: T,
        clazz: KClass<*>
    ): T = getTypedPropertyOrNull<T>(key, clazz) ?: defaultValue

    @PublishedApi
    internal fun <T> getTypedPropertiesOrDefault(
        key: String,
        defaultValue: List<T>,
        clazz: KClass<*>
    ): List<T> = getTypedPropertiesOrNull<T>(key, clazz) ?: defaultValue

    @PublishedApi
    internal fun <T> getTypedPropertyOrThrow(
        key: String,
        clazz: KClass<*>
    ): T = getTypedPropertyOrNull<T>(key, clazz) ?: throw KeyNotFoundException("Property $key does not exist")

    @PublishedApi
    internal fun <T> getTypedPropertiesOrThrow(
        key: String,
        clazz: KClass<*>
    ): List<T> = getTypedPropertiesOrNull<T>(key, clazz) ?: throw KeyNotFoundException("Property $key does not exist")

    @PublishedApi
    internal fun <T> getTypedPropertyOrNull(
        key: String,
        clazz: KClass<*>
    ): T? {
    ...
    @PublishedApi
    internal fun <T> getTypedPropertiesOrNull(
        key: String,
        clazz: KClass<*>
    ): List<T>? {
