...
inline fun <reified T> Application.inject(key: String): Lazy<T> = lazy { context().getTypedPropertyOrThrow<T>(key, T::class) }
...
inline fun <reified T> Application.injectOrNull(key: String): Lazy<T?> = lazy { context().getTypedPropertyOrNull<T>(key, T::class) }

inline fun <reified T> Application.injectList(key: String): Lazy<List<T>> = lazy { context().getTypedPropertiesOrThrow<T>(key, T::class) }

inline fun <reified T> Application.injectList(
    key: String,
    defaultValue: List<T>
